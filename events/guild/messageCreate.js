...
module.exports = async (client, message) => {
  ...
  if (!cmd || cmd.length == 0) {
    if (mPrefix.includes(client.user.id)) {
      message.reply({
        embeds: [new Discord.EmbedBuilder().setColor(ee.color).setFooter({ text: ee.footertext, iconURL: ee.footericon}).setTitle(`:thumbsup: **My Prefix here, is __\`${prefix}\`__**`)]
      })
    }
    return;
  }
  ...
  if (command) {
    ...
    //Check if user is on cooldown with the cmd, with Tejas Lamba#1924's Function from /handlers/functions.js
